...
class TourManager:
    ...
    def retirer_tour_par_position(self, position: Position) -> None:
        ...
        self.tours = [
            ...
            if not (int(t.position.x) == int(position.x) and int(t.position.y) == int(position.y))
        ]
                    ...
                    if hasattr(pr, "aTouche") and pr.aTouche(e):
                        if hasattr(pr, "appliquerDegats"):
                            ...
                            if isinstance(pr, ProjectileTourMage):
                                ...
                                if isinstance(pr, ProjectileFleche) and hasattr(e, "__class__") and "Chevalier" in str(e.__class__):
    ...
    def mettre_a_jour_feux_de_camps(self, dt: float, nuit_surface: pygame.Surface | None = None) -> None:
    ...
    def dessiner_range_tour(self, ecran: pygame.Surface, tour_selectionnee: Tuple[int, int] | None, taille_case: int) -> None:
    ...
    def creer_tour(self, type_tour: str, position: Position, tour_id: int) -> Tour | None:
    ...
    def peut_placer_tour(self, case: tuple[int, int], type_tour: str, cases_bannies: set) -> bool:
    ...
    def dessiner_tours_placees(self, ecran: pygame.Surface, taille_case: int) -> None:
        ...
        for (x_case, y_case), data in self.positions_occupees.items():
            ...
            if tour and hasattr(tour, "dessiner"):
                ...
                ttype = data["type"]
                surf = None
                if ttype in self.game.tower_assets and self.game.tower_assets[ttype]["frames"]:
